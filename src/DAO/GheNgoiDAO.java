...
public class GheNgoiDAO {
    ...
    public static boolean tao(GheNgoi gheNgoi) {
        ...
        try (Connection conn = DbConnect.getConnection();
            ...
            stmt.setString(1, gheNgoi.getMaGhe());	//lấy gtri gán cho lệnh sql
            ...
            return stmt.executeUpdate() > 0;	//ít nhất 1 hàng thay đổi là thành công, return	
        	...
        	//Unique constraint (Ràng buộc duy nhất)
             if (e.getMessage().toLowerCase().contains("duplicate entry") || e.getMessage().toLowerCase().contains("unique constraint")) { //gặp lỗi duplicate entry và unique constraint trong sql
    ...
    public static List<GheNgoi> readAll() {
        List<GheNgoi> list = new ArrayList<>();	//ds ghế
        ...
        try (Connection conn = DbConnect.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {	//thực thi câu lệnh - lấy kq dạng resultSet	

            while (rs.next()) {
                String maGhe = rs.getString("maGhe");
                PhongChieu phongChieu = new PhongChieu(	//tạo đt phongChieu
                ...
                list.add(new GheNgoi(maGhe, phongChieu, hang, soGhe, trangThai)); //add gtri vào GHeNgoi
            }
        } catch (SQLException e) {
    ...
    public static List<GheNgoi> timTheoMaPhongChieu(String maPhongSearch) {
        ...
        try (Connection conn = DbConnect.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {	//duyệt các hàng để add vào ds
                while (rs.next()) {
                    String maGhe = rs.getString("maGhe");
    ...
    public static GheNgoi timTheoMaGhe(String maGhe) {
